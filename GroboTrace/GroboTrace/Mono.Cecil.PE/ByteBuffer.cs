...
namespace GroboTrace.Mono.Cecil.PE {
    ...
    unsafe class RawByteBuffer
    {
        ...
        public byte[] ReadBytes(int length)
        {
            ...
            Marshal.Copy((IntPtr)(buffer + position), bytes, 0, length);
            position += length;
            return bytes;
        }
        ...
        public float ReadSingle()
        {
            ...
            float value = *(float*)(buffer + position);
            position += 4;
            return value;
        }
...
//            double value = BitConverter.ToDouble(buffer, position);
            double value = *(double*)(buffer + position);
            position += 8;
            return value;
        }
